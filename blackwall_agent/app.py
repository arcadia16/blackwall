...
from agent.config import MASTER_SERVER_IP, MASTER_SERVER_PORT
...
if __name__ == '__main__':
    try:
        response = r_post(f"http://{MASTER_SERVER_IP}:{MASTER_SERVER_PORT}/agent/{AGENT_ID}",
    ...
    uvicorn.run("app:app", host="0.0.0.0", port=8091, log_level="info",
                ssl_certfile="agent/certs/public_certificate.pem",
                ssl_keyfile="agent/certs/private_key.pem")
