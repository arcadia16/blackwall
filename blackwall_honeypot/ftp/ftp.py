...
class SimpleFTPProtocol(basic.LineReceiver):
    ...
    def lineReceived(self, line):
        ...
        if command == "HELP":
            self.sendLine(b"214 System help message not set")
        if command == "STAT":
            self.sendLine(b"211 System status unknown")
        if command == "SYST":
            self.sendLine(b"215 Linux system type.")
        if command == "USER":
            self.last_user = line_str.split(" ")[1]
            self.userReceived = True
            self.sendLine(b"331 Username okay, need password")
        elif command == "PASS" and self.userReceived:
            print(f"FTP::Login attempt::{self.transport.getPeer().host}::{self.last_user}:{line_str.split(" ")[1]}")
            self.attempts += 1
...
def main():
    ...
    parser.add_argument(
        "--port", type=int, default=21, help="Port to bind the FTP server to."
    )
